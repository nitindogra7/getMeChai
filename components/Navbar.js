...
const Navbar = () => {
  ...
  if (status === "loading") {
    return (
      <nav
        className={`fixed top-0 w-full bg-white/70 backdrop-blur-xl border-b border-stone-200/50 shadow-[0_4px_30px_rgba(0,0,0,0.03)] z-50 h-16 sm:h-18`}
      ></nav>
    );
  }
  ...
  return (
    <nav
      className={`fixed top-0 w-full transition-transform ${scrollDir === "down" ? "-translate-y-full" : "translate-y-0"} bg-white/70 backdrop-blur-xl border-b border-stone-200/50 shadow-[0_4px_30px_rgba(0,0,0,0.03)] z-50 flex h-16 sm:h-18 items-center text-lg justify-between font-medium px-1 md:px-4`}
    >
      <div className="sm:hidden flex items-center gap-3 pl-2">
        <button
          ...
          className="flex items-center justify-center p-1 rounded-lg hover:bg-stone-100 transition-colors"
        >
          {!session && (
            <span className="material-symbols-outlined !text-[26px] text-stone-700">
              menu
            </span>
          )}
        </button>
        <Link href={session?.user?.name ? "/home" : "/"}>
          <object className="w-9 h-10 pointer-events-none" data="/images/tea.png"></object>
        </Link>
      </div>

      {!session && (
        <ul
          className={`fixed top-0 left-0 h-screen w-screen flex flex-col gap-7 py-4 px-4 bg-white/95 backdrop-blur-xl text-2xl z-auto
